<ul class="thumbnails">
    <?php
    $ctwo_id = ot_get_option('product_tab2_cat');
    if( $ctwo_id )
    {
        $b_args = array(
    ...
    {
        $b_args = array(  'post_type'  => 'wpsc-product', 'posts_per_page'  => -1, 'post_status' => 'publish' );
    }
    $tab2_query = new WP_Query( $b_args );
    if ( $tab2_query->have_posts() ):
        while ( $tab2_query->have_posts() ) :
            $tab2_query->the_post();
            ?>
            <li class="span2">
                <div class="thumbnail">
